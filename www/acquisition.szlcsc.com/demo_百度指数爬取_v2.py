...
class BaiduIndex:
    ...
    def get_index(self):
        ...
        while 1:
            try:
                ...
                generalRatio_encrypt_datas,encrypt_datas, uniqid = self._get_encrypt_datas(
                ...
                for encrypt_data in encrypt_datas:
                    ...
                    for formated_data in self._format_data(encrypt_data,generalRatio_encrypt_datas[0]):
                        yield formated_data
    ...
    def _get_encrypt_datas(self, start_date, end_date, keywords):
        ...
        generalRatio_encrypt_datas = []
        for single_data in datas['data']['generalRatio']:
            generalRatio_encrypt_datas.append(single_data)
        return (generalRatio_encrypt_datas,encrypt_datas, uniqid)
    ...
    def _format_data(self, data,generalRatio_datas):
        """
            格式化堆在一起的数据
        """
        ...
        for i in range(time_length):
            for kind in self._all_kind:
                ...
                formated_data = {
                    ...
                    '整体日均值':generalRatio_datas['all']['avg'],
                    '移动日均值': generalRatio_datas['wise']['avg'],
                    'pc日均值': generalRatio_datas['pc']['avg'],
                    '整体同比': str(generalRatio_datas['all']['yoy'])+'%',
                    '整体环比': str(generalRatio_datas['all']['qoq']) + '%',
                    '移动同比': str(generalRatio_datas['wise']['yoy']) + '%',
                    '移动环比': str(generalRatio_datas['wise']['qoq']) + '%',
                    'pc同比': str(generalRatio_datas['pc']['yoy']) + '%',
                    'pc环比': str(generalRatio_datas['pc']['qoq']) + '%',
                }
                yield formated_data
...
if __name__ == "__main__":
    ...
    # writer.writerow(('name', 'type','value', 'date','整体日均值','移动日均值','整体同比','整体环比','移动同比','移动环比','pc同比','pc环比'))
